...
// ─── Budget category → weight mapping
...
export default function InflationPage() {
  ...
  const budgetDocRef = user && firestore ? doc(firestore, 'budgets', user.uid) : null;
  const budgetDoc = useDoc(budgetDocRef);
  ...
  // Monthly cost impact estimate (using average PT household spend ~2200€/month)
      ...
      <div className="container max-w-5xl mx-auto py-8 px-4 space-y-8">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">A Tua Inflação Real</h1>
          ...
          {!loading && data.isLive && (
            <Badge variant="outline" className="text-xs bg-green-50 border-green-200 text-green-700 dark:bg-green-900/20 dark:text-green-400">
              ✓ Dados em tempo real — {data.periodLabel} — {data.source}
            </Badge>
          )}
          {!loading && !data.isLive && (
            <Badge variant="outline" className="text-xs bg-amber-50 border-amber-200 text-amber-700">
              Dados de referência — {data.periodLabel}
            </Badge>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card className="border-red-200 dark:border-red-900/30 bg-red-50/50 dark:bg-red-900/10">
            <CardContent className="pt-5 pb-4">
              <p className="text-xs uppercase tracking-widest text-muted-foreground mb-1">Inflação oficial</p>
              <p className="text-4xl font-black text-red-600 dark:text-red-400">{officialRate.toFixed(1)}<span className="text-xl">%</span></p>
              <p className="text-xs text-muted-foreground mt-1">Cabaz médio INE (IHPC)</p>
            </CardContent>
          </Card>
          <Card className={cn('border-2', diff > 0.5 ? 'border-orange-400 bg-orange-50/50 dark:bg-orange-900/10' : 'border-green-400 bg-green-50/50 dark:bg-green-900/10')}>
            <CardContent className="pt-5 pb-4">
              <p className="text-xs uppercase tracking-widest text-muted-foreground mb-1">A tua inflação real</p>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {diff > 0.5 ? `+${diff.toFixed(1)}% acima da média` : diff < -0.5 ? `${diff.toFixed(1)}% abaixo da média` : 'Próxima da média nacional'}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-5 pb-4">
              ...
              <p className="text-4xl font-black text-foreground">
                +{monthlyImpact.toFixed(0)}<span className="text-xl">€</span>
              </p>
              <p className="text-xs text-muted-foreground mt-1">Sobre despesa mensal média (2.200€)</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              O cabaz oficial esconde variações enormes entre categorias. Ajusta os pesos abaixo para ver a tua inflação real.
            </CardDescription>
          </CardHeader>
            ...
            {!user && (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-sm">
                ...
                <span className="text-blue-700 dark:text-blue-300">
                  Tens o orçamento preenchido?{' '}
                  <Link href="/login" className="font-semibold underline">Inicia sessão</Link>
            ...
            {budgetLoaded && (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-sm">
                ...
                <span className="text-green-700 dark:text-green-300">
                  Pesos importados automaticamente do teu orçamento.
                </span>
              </div>
            )}
                  ...
                  <div key={cat.coicop} className="grid grid-cols-1 sm:grid-cols-[auto_1fr_auto] gap-x-4 gap-y-1 items-start">
                    ...
                    <div className="text-right">
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p className="text-xs">Classificação INE/Eurostat</p>
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              O que o número oficial esconde
            </CardTitle>
            <CardDescription>
              Existe uma diferença entre os indicadores macro e o seu impacto real no cidadão comum.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="border-dashed bg-gradient-to-br from-blue-50/50 to-purple-50/50 dark:from-blue-900/10 dark:to-purple-900/10">
          <CardContent className="pt-6 pb-6 text-center space-y-3">
            ...
            <h3 className="font-semibold text-lg">Vê como a inflação come o teu orçamento</h3>
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
              Preenche o teu orçamento pessoal e esta página calcula automaticamente a inflação real do teu perfil de consumo.
            </p>
        ...
        <p className="text-xs text-muted-foreground text-center pb-4">
          Dados: Eurostat IHPC (série prc_hicp_manr, Portugal) · INE · Banco de Portugal.<br />
          Os valores "reais" são estimativas baseadas em fontes alternativas e não substituem os dados oficiais. <a href="/methodology" className="underline hover:text-primary">Metodologia</a>
        </p>
