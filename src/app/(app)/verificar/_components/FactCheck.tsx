...
// Mapeia qualquer valor legado (PT, capitalizado, etc.) para a chave enum can├│nica
const VERDICT_NORMALIZE: Record<string, string> = {
  ...
  // portugu├¬s (legado Firestore)
  'verdadeiro': 'true', 'falso': 'false', 'enganador': 'misleading',
  'sem evid├¬ncia': 'no_evidence', 'sem evidencia': 'no_evidence', 'no evidence': 'no_evidence',
};
...
function FactCheckResultDisplay({ result, claim }: { result: FactCheckOutput, claim: string }) {
  ...
  useEffect(() => {
    if (language === 'en' && result) {
      const tVerdict = translationCache.get(`${result.verdict}:en`);
      const tExpl = translationCache.get(`${result.explanation}:en`);
      if (tVerdict || tExpl) {
  ...
  const handleTranslate = () => {
    startTransition(async () => {
      ...
      setShowOriginal(false);
      translationCache.set(`${result.verdict}:en`, resVerdict);
      translationCache.set(`${result.explanation}:en`, resExpl);
    });
  };
  ...
  // Always use i18n translation for the verdict label ΓÇö never show the raw enum value
...
export default function FactCheckPage() {
  ...
  const handleFactCheck = useCallback((customClaim?: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        const cached = await fetch(`/api/data/publicFactChecks/${claimId}`);
        if (cached.ok) {
          const data = await cached.json();
          if (data) { setResult(data as FactCheckOutput); return; }
        }

        const res = await getFactCheck({ claim: textToUse }, language as Language);
        setResult(res);

        dbSet('publicFactChecks', claimId, { claim: textToUse, ...res, createdAt: nowTs() }).catch(() => {});
        if (user) {
          dbSet('user_factChecks', claimId, { userId: user.uid, claim: textToUse, ...res, createdAt: nowTs() }).catch(() => {});
        }
      } catch (e: any) {
  ...
  const handleDeleteHistory = async (id: string) => {
    if (!user) return;
    try {
      await dbDelete('user_factChecks', id);
