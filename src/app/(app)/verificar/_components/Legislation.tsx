...
import { useUser, useCollection, dbSet, dbDelete, nowTs } from '@/firebase';
...
function LegislationResultDisplay({ result, questionText }: { result: ConsultLegislationOutput, questionText: string }) {
  ...
  useEffect(() => {
    if (language === 'en' && result) {
      const tAnswer = translationCache.get(`${result.answer}:en`);
      if (tAnswer) { setTranslated(tAnswer); setShowOriginal(false); }
    } else {
      setTranslated(null);
  ...
  const handleTranslate = () => {
    startTransition(async () => {
      ...
      translationCache.set(`${result.answer}:en`, res);
    });
  };
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <SocialShare
              url={shareUrl}
              title={`Legisla├º├úo: "${questionText.substring(0, 60)}${questionText.length > 60 ? '...' : ''}"`}
            />
...
export default function LegislationPage() {
  ...
  const handleConsultation = useCallback((customQuestion?: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        const cached = await fetch(`/api/data/publicLegislationQueries/${id}`);
        if (cached.ok) {
          const data = await cached.json();
          if (data) { setResult(data as ConsultLegislationOutput); return; }
        }

        const response = await getLegislationInfo({ question: textToUse }, language);
        setResult(response);

        dbSet('publicLegislationQueries', id, { question: textToUse, ...response, createdAt: nowTs() });
        if (user) {
          dbSet('user_legislationQueries', id, { userId: user.uid, question: textToUse, ...response, createdAt: nowTs() });
        }
      } catch (e: any) {
  ...
  const { data: pastQueries } = useCollection<LegislationQuery>('user_legislationQueries', user ? { where: [['userId', '==', user.uid]], orderBy: 'createdAt', orderDir: 'desc', limit: 10 } : undefined);
  const { data: recentQueries, isLoading: isLoadingPublic } = useCollection<LegislationQuery>('publicLegislationQueries', { orderBy: 'createdAt', orderDir: 'desc', limit: 10 });

  const handleDeleteHistory = async (id: string) => {
    if (!user) return;
    try {
      await dbDelete('user_legislationQueries', id);
