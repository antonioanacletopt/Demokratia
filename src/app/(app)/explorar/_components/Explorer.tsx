...
import { useCollection, useUser, dbAdd, dbSet, nowTs } from '@/firebase';
...
function transformToChartData(rawData: any[]): DataPoint[] {
  ...
  const labelKey = keys.find(k => /ano|date|year|m├¬s|mes|month|categoria|category|pa├¡s|country|tipo|type|entidade|label|nome|name|escala|size/i.test(k)) || keys[0];
  const valueKey = keys.find(k => /valor|value|total|taxa|rate|quantidade|amount|Γé¼|%|pib|gdp|├¡ndice|indice|m├⌐dicos|doctors|empresas|companies/i.test(k)) 
...
function UniversalDataCard({ 
  ...
  useEffect(() => {
    if (language === 'en' && (title || description)) {
      const cachedTitle = translationCache.get(`${title}:en`);
      const cachedDesc = translationCache.get(`${description}:en`);
      if (cachedTitle || cachedDesc) {
        setTranslated({ title: cachedTitle || title, desc: cachedDesc || description });
  ...
  const handleTranslate = () => {
    startTransition(async () => {
      ...
      setShowOriginal(false);
      if (title.length <= MAX_CACHE_LENGTH) translationCache.set(`${title}:en`, resTitle);
      if (description.length <= MAX_CACHE_LENGTH) translationCache.set(`${description}:en`, resDesc);
    });
  };
...
export default function ExplorerPage() {
  ...
  const performSearch = useCallback(async (text: string) => {
    ...
    try {
      ...
      // Check public cache
      const cacheResp = await fetch(`/api/data/publicStatisticQueries/${docId}`);
      if (cacheResp.ok) {
        const cachePayload = await cacheResp.json() as { data?: UniversalData };
        if (cachePayload?.data) {
          setAiResponse(cachePayload.data);
      ...
      if (finalResult) {
        ...
        dbSet('publicStatisticQueries', docId, { ...finalResult, createdAt: nowTs() }).catch(() => {});
      } else {
        setNoResults(true);
      }
    } catch (e) {
  ...
  const { data: datasets, isLoading } = useCollection<any>('statisticalData');

  const { data: officialSources } = useCollection<any>('dataSources', {
  ...
  const handleSuggestSource = async () => {
    if (!user || !suggestName.trim() || !suggestUrl.trim()) return;
    setIsSuggesting(true);
    try {
      await dbAdd('dataSources', {
        name: suggestName,
        url: suggestUrl,
                  ...
                  {ds.map((d: any) => {
                    ...
                    return (
                      <UniversalDataCard 
                        ...
                        source={source}
                        unit={title.includes('%') ? '%' : (title.includes('Γé¼') ? 'Γé¼' : '')}
                        data={transformed} 
                      />
                    );
                  })}
