...
export default function IRSPage() {
  ...
  const budgetConfigRef = useMemoFirebase(
    () => user && firestore ? doc(firestore, 'users', user.uid, 'budget', `config_${today.getFullYear()}_${today.getMonth() + 1}`) : null,
    [user, firestore]
  );
  const { data: budgetConfig } = useDoc<any>(budgetConfigRef);
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        <div className="lg:col-span-8 space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            {/* Income Card */}
            <Card className="shadow-md border-primary/10"><CardHeader className="bg-primary/5 py-4"><CardTitle className="text-lg flex items-center gap-2"><Landmark className="h-5 w-5 text-primary" /> {t('irs.incomeCard')}</CardTitle></CardHeader><CardContent className="pt-6 space-y-4"><div className="space-y-2"><Label>{t('irs.catA')}</Label><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.categoryA} onChange={(e) => setIncome(prev => ({ ...prev, categoryA: Number(e.target.value) }))} className="pr-8" /></div></div><div className="space-y-2"><Label>{t('irs.catB')}</Label><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.categoryB} onChange={(e) => setIncome(prev => ({ ...prev, categoryB: Number(e.target.value) }))} className="pr-8" /></div></div><div className="space-y-2"><Label>{t('irs.propertyInc')}</Label><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.property} onChange={(e) => setIncome(prev => ({ ...prev, property: Number(e.target.value) }))} className="pr-8" /></div><div className="flex items-center space-x-2 pt-1"><Switch id="englobe-property" checked={englobePropertyIncome} onCheckedChange={setEnglobePropertyIncome} /><Label htmlFor="englobe-property" className="text-sm font-normal">{t('irs.englobeProp')}</Label></div></div><div className="space-y-2"><Label>{t('irs.capitalInc')}</Label><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.capital} onChange={(e) => setIncome(prev => ({ ...prev, capital: Number(e.target.value) }))} className="pr-8" /></div><div className="flex items-center space-x-2 pt-1"><Switch id="englobe-capital" checked={englobeCapitalIncome} onCheckedChange={setEnglobeCapitalIncome} /><Label htmlFor="englobe-capital" className="text-sm font-normal">{t('irs.englobeCap')}</Label></div></div><Separator/><div className="space-y-2"><Label>{t('irs.retention')}</Label><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={retention} onChange={(e) => setRetention(Number(e.target.value))} className="pr-8 border-accent/20" /></div></div></CardContent></Card>
          </div>
