      ...
      <div className="grid gap-8 lg:grid-cols-12">
        <div className="lg:col-span-8 space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card className="shadow-md"><CardHeader className="bg-muted/30 py-4"><CardTitle className="text-lg flex items-center gap-2"><UserCircle className="h-5 w-5 text-primary" /> {t('irs.personalCard')}</CardTitle></CardHeader><CardContent className="pt-6 space-y-4"><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.maritalStatus')}</Label><InfoPopover title="Quociente Familiar" content="O estado civil define como o rendimento é dividido para aplicação das taxas. Casados podem optar por tributação conjunta (Art. 69.º CIRS)." /></div><Select value={maritalStatus} onValueChange={(v: any) => setMaritalStatus(v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Single">{t('irs.single')}</SelectItem><SelectItem value="Married_Joint">{t('irs.marriedJoint')}</SelectItem><SelectItem value="Married_Separate">{t('irs.marriedSeparate')}</SelectItem></SelectContent></Select></div><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.dependents')}</Label><InfoPopover title="Dedução por Dependente" content="Cada dependente confere uma dedução fixa à coleta (Art. 78.º-A do CIRS)." /></div><Input type="number" value={dependents} onChange={(e) => setDependents(Number(e.target.value))} min={0} max={10} /></div><Separator/><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.incapacity')}</Label><InfoPopover title="Incapacidade" content="A dedução à coleta por pessoa com deficiência é de 4x o IAS (Indexante dos Apoios Sociais). (Art. 87.º CIRS)." /></div><div className="space-y-2 rounded-md border p-3"><div className="flex items-center justify-between"><Label htmlFor="taxpayer-disability" className="text-sm font-normal">{t('irs.taxpayer')}</Label><Switch id="taxpayer-disability" checked={taxpayerDisability} onCheckedChange={setTaxpayerDisability}/></div>{maritalStatus.startsWith('Married') && <div className="flex items-center justify-between"><Label htmlFor="spouse-disability" className="text-sm font-normal">{t('irs.spouse')}</Label><Switch id="spouse-disability" checked={spouseDisability} onCheckedChange={setSpouseDisability}/></div>}<div className="flex items-center justify-between"><Label htmlFor="dependents-disability" className="text-sm font-normal">{t('irs.dependents')}</Label><Input id="dependents-disability" type="number" value={dependentsWithDisability} onChange={(e) => setDependentsWithDisability(Number(e.target.value))} min={0} max={dependents} className="w-24 h-8"/></div></div></div></CardContent></Card>

            {/* Income Card */}
            <Card className="shadow-md border-primary/10"><CardHeader className="bg-primary/5 py-4"><CardTitle className="text-lg flex items-center gap-2"><Landmark className="h-5 w-5 text-primary" /> {t('irs.incomeCard')}</CardTitle></CardHeader><CardContent className="pt-6 space-y-4"><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.catA')}</Label></div><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.categoryA} onChange={(e) => setIncome(prev => ({ ...prev, categoryA: Number(e.target.value) }))} className="pr-8" /></div></div><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.catB')}</Label></div><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.categoryB} onChange={(e) => setIncome(prev => ({ ...prev, categoryB: Number(e.target.value) }))} className="pr-8" /></div></div><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.propertyInc')}</Label></div><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.property} onChange={(e) => setIncome(prev => ({ ...prev, property: Number(e.target.value) }))} className="pr-8" /></div><div className="flex items-center space-x-2 pt-1"><Switch id="englobe-property" checked={englobePropertyIncome} onCheckedChange={setEnglobePropertyIncome} /><Label htmlFor="englobe-property" className="text-sm font-normal">{t('irs.englobeProp')}</Label></div></div><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.capitalInc')}</Label></div><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={income.capital} onChange={(e) => setIncome(prev => ({ ...prev, capital: Number(e.target.value) }))} className="pr-8" /></div><div className="flex items-center space-x-2 pt-1"><Switch id="englobe-capital" checked={englobeCapitalIncome} onCheckedChange={setEnglobeCapitalIncome} /><Label htmlFor="englobe-capital" className="text-sm font-normal">{t('irs.englobeCap')}</Label></div></div><Separator/><div className="space-y-2"><div className="flex items-center"><Label>{t('irs.retention')}</Label></div><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={retention} onChange={(e) => setRetention(Number(e.target.value))} className="pr-8 border-accent/20" /></div></div></CardContent></Card>
          </div>

          <Card className="shadow-md"><CardHeader className="bg-muted/30 py-4 flex flex-row items-center justify-between"><CardTitle className="text-lg flex items-center gap-2"><TrendingUp className="h-5 w-5 text-blue-600" /> Mais-Valias (Categoria G)</CardTitle><Switch checked={hasCapitalGains} onCheckedChange={setHasCapitalGains} /></CardHeader>{hasCapitalGains && (<CardContent className="pt-6 space-y-4 animate-in fade-in duration-300"><div className="space-y-2"><Label>Valor de Venda do Imóvel</Label><Input type="number" value={capitalGains.realizationValue} onChange={(e) => setCapitalGains(prev => ({ ...prev, realizationValue: Number(e.target.value) }))} /></div><div className="space-y-2"><Label>Valor de Aquisição do Imóvel</Label><Input type="number" value={capitalGains.acquisitionValue} onChange={(e) => setCapitalGains(prev => ({ ...prev, acquisitionValue: Number(e.target.value) }))} /></div><div className="space-y-2"><Label>Despesas e Encargos</Label><Input type="number" value={capitalGains.expenses} onChange={(e) => setCapitalGains(prev => ({ ...prev, expenses: Number(e.target.value) }))} /></div><Separator/><div className="flex items-center space-x-2 pt-1"><Switch id="is-primary" checked={capitalGains.isPrimaryResidence} onCheckedChange={(val) => setCapitalGains(prev => ({ ...prev, isPrimaryResidence: val }))} /><Label htmlFor="is-primary" className="text-sm font-normal">Venda de Habitação Própria e Permanente</Label></div>{capitalGains.isPrimaryResidence && (<div className="space-y-2 pl-6 border-l-2 border-primary/20 animate-in fade-in duration-300"><Label>Valor Reinvestido</Label><Input type="number" value={capitalGains.reinvestmentValue} onChange={(e) => setCapitalGains(prev => ({ ...prev, reinvestValue: Number(e.target.value) }))} /></div>)}</CardContent>)}</Card>

          <Card className="shadow-md"><CardHeader className="bg-muted/30 py-4"><CardTitle className="text-lg flex items-center gap-2"><Rocket className="h-5 w-5 text-accent" /> Regimes Especiais e Investimentos</CardTitle></CardHeader><CardContent className="grid gap-6 sm:grid-cols-2 pt-6"><div className="space-y-2"><Label>IRS Jovem</Label><Select value={irsJovemYear} onValueChange={(v: any) => setIrsJovemYear(v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Nenhum">Nenhum</SelectItem><SelectItem value="Ano 1">1º Ano</SelectItem><SelectItem value="Ano 2">2º Ano</SelectItem><SelectItem value="Ano 3">3º Ano</SelectItem><SelectItem value="Ano 4">4º Ano</SelectItem><SelectItem value="Ano 5">5º Ano</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label>Contribuições PPR</Label><div className="relative"><span className="absolute right-3 top-2.5 text-muted-foreground text-sm">€</span><Input type="number" value={ppr} onChange={(e) => setPpr(Number(e.target.value))} className="pr-8" /></div></div></CardContent></Card>
