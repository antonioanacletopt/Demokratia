 ...
 * Utilizes the definePrompt pattern for stable model registration in Next.js.
 */
...
export async function getEconomicSimulation(input: { policyDescription: string }, lang: Language = 'pt') {
  const { output } = await simulationPrompt({ policyDescription: input.policyDescription, lang: lang === 'en' ? 'English' : 'Portuguese' });
  return output!;
}
